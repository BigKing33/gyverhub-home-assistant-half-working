"""Text platform for GyverHub integration."""
...
from .protocol import GyverHubDevice, InputWidget
...
async def async_setup_entry(
    ...
    """Set up GyverHub text entities from a config entry."""
    ...
    def async_add_entities_for_device(device: GyverHubDevice) -> None:
        """Add text entities for a device."""
        new_entities = []
        
        for input_widget in device.inputs:
            unique_id = f"{device.unique_id}_{input_widget.id}"
            if unique_id not in added_entities:
                added_entities.add(unique_id)
                new_entities.append(
                    GyverHubTextInput(coordinator=coordinator, device=device, widget=input_widget)
                )
        
        if new_entities:
            _LOGGER.info(
                "Adding %d text entities for device %s",
                len(new_entities),
                device.name
            )
...
class GyverHubTextInput(TextEntity):
    """Representation of a GyverHub text input."""
    ...
    def __init__(
        ...
        if widget.max_length > 0:
            self._attr_native_max = widget.max_length
    ...
    def _handle_ui_update(self, device: GyverHubDevice) -> None:
        ...
        if device.device_id == self._device.device_id:
            # Find updated widget
            for input_widget in device.inputs:
                if input_widget.id == self._widget.id:
                    self._widget = input_widget
